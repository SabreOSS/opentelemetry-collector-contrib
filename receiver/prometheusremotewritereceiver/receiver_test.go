...
func TestPrometheusRemoteWriteReceiver(t *testing.T) {
	...
	r, err := MakeRequest(context.Background(), "http://localhost:19291", request)
	...
	require.Equal(t, pmetric.MetricTypeNone, cms.AllMetrics()[0].ResourceMetrics().At(3).ScopeMetrics().At(0).Metrics().At(0).Type())
}
